...
namespace HandyHelpers.Tests
{
    ...
    public class IntHelpersTests
    {
        ...
        [InlineData(-10000)]
        [InlineData(int.MinValue)]
        public async Task TestTimesNegativeOrZeroWithAsyncCallback(int value)
        {
            var list = new List<int>();

            var iterations = await value.Times(async i =>
            {
                await Task.Run(() => list.Add(i));
                return true;
            });
        ...
        [InlineData(100)]
        [InlineData(10000)]
        public async Task TestTimesPositiveSmallDataWithAsyncCallback(int value)
        {
            var list = new List<int>();
        ...
        [Fact]
        public async Task TestTimesPositiveBigDataWithAsyncCallback()
        {
            var list = new List<int>();
            var stopPoint = 2000000;
            var iterations = await int.MaxValue.Times(async i =>
            {
                await Task.Run(() => list.Add(i));
                return i < stopPoint - 1;
            });
        ...
        [Fact]
        public async Task TestTimesPositiveWithExceptionAndAsyncCallback()
        {
            var list = new List<int>();
            var stopPoint = 20;
            var exceptionText = "Stop point exception!";
            ...
            try
            {
                var iterations = await 100.Times(async i =>
                {
                    await Task.Run(() => list.Add(i));
